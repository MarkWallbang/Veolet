...
func ssmain() {
	...
	fmt.Print(targetaddress)
	config := GetConfig("testnet")
	fmt.Print(config)
	fmt.Print("asasdasdasdasd")
	//fmt.Print(FetchBalance(*config, targetaddress))

	/*
		address, _ := CreateNewAccount(*config)
		fmt.Print(address)

		res := MintToken(*config, address, "https://veoletimages.s3.eu-central-1.amazonaws.com/susi.jpeg", "Susi", address, "Susis NFT", "0784fb1h3", 1)
		fmt.Print(res)*/
}
...
func oldmain() {
	...
	cap, used := lib.FetchStorageCapacity(*config, serviceaddress)
	fmt.Print(cap, int(used))
